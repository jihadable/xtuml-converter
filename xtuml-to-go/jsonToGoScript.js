...
// Mengarahkan ke halaman bantuan
document.getElementById('help-icon').addEventListener('click', function () {
	alert("1. Upload a JSON file via the Upload JSON File button.\n2. The application reads and displays the JSON content.\n3. Click the Translate button to process and generate Go structs.\n4. The translated Go code is displayed in the output panel.\n5. Download or copy the resulting code as needed.\n")
});

// Mengarahkan ke halaman informasi
document.getElementById('info-icon').addEventListener('click', function () {
	alert("JSON to Go Compiler is a web application designed to assist developers in converting JSON data structures into Go (Golang) code automatically.\nThis application aims to speed up the development process for Go-based applications that require JSON data mapping to Go structs, minimizing manual coding errors.")
});
    ...
    if (file && file.name.endsWith(".json")) {
        ...
        reader.onload = function (e) {
            try {
                ...
                console.log("File JSON berhasil dibaca:", jsonData); // Debugging log
...
if (typeof module != 'undefined') {
	if (!module.parent) {
		...
		if (filename) {
			...
			const json = fs.readFileSync(filename, 'utf8');
			jsonToGoWithErrorHandling(json);
			process.exit(0); // Gunakan ini untuk menghentikan program dengan aman
		}
...
document.getElementById("translateBtn").addEventListener("click", translateToGo);

function translateToGo() {
    if (!jsonData) {
    ...
    const result = jsonToGo(jsonString, "Root"); // Menggunakan fungsi jsonToGo dari file yang diunggah

    if (result.error) {
        console.error("Error dalam konversi JSON ke Go:", result.error);
        alert("Error converting JSON to Go. Please check your JSON file.");
        return;
    }

    document.getElementById("goOutput").textContent = result.go;

    const buttons = document.querySelectorAll('.button-group button');
    buttons.forEach(button => button.style.display = "block");

    console.log("Translation successful!"); // Debugging log
}


function processEntity(entityName, entity, goCode, processedEntities) {
    console.log(`Processing entity: ${entityName}`); // Debugging log

    const entityNameCapitalized = capitalizeFirstLetter(entityName);

    if (processedEntities.has(entityNameCapitalized)) {
        console.log(`Skipping already processed entity: ${entityNameCapitalized}`); // Debugging log
        return goCode;
    }
    processedEntities.add(entityNameCapitalized);

    goCode += `// ${entityNameCapitalized} represents a ${entityName} type\n`;
    goCode += `type ${entityNameCapitalized} struct {\n`;

    // Proses attributes
    if (entity.attributes && Array.isArray(entity.attributes)) {
        entity.attributes.forEach(attr => {
            goCode += `    ${capitalizeFirstLetter(attr.name)} ${convertType(attr.type)}\n`;
        });
    }

    // Proses nested entities
    if (entity.nested && Array.isArray(entity.nested)) {
        entity.nested.forEach(nestedEntity => {
            const nestedNameCapitalized = capitalizeFirstLetter(nestedEntity.name);
            goCode += `    ${nestedNameCapitalized} ${nestedNameCapitalized}\n`;
        });
    }

    goCode += `}\n\n`; // Penutup struct utama

    // Rekursi untuk nested entities
    if (entity.nested && Array.isArray(entity.nested)) {
        entity.nested.forEach(nestedEntity => {
            goCode = processEntity(nestedEntity.name, nestedEntity, goCode, processedEntities);
        });
    }
...
function downloadGoCode() {
    const goCode = document.getElementById("goOutput").textContent;
    
    if (!goCode.trim()) {
        alert("Go code is empty. Please translate JSON data first.");
        return;
    }
    ...
    link.download = "output.go"; // Nama file output
    document.body.appendChild(link); // Menambahkan elemen ke DOM
    link.click(); // Menjalankan klik untuk memulai unduhan
    document.body.removeChild(link); // Menghapus elemen setelah unduh
}

// Function to copy the Go code to clipboard
function copyGoCode() {
    const goCode = document.getElementById("goOutput").textContent;
    if (goCode && goCode !== "The Go code will appear here...") {
...
// Function to reload the page (Home button)
function goHome() {
    location.reload();
}